...
namespace IngameDebugConsole
{
	...
	public class DebugLogManager : MonoBehaviour
	{
        ...
        [SerializeField]
        private List<GameObject> notSearchBtn;
		...
		{            
			...
			else if(!isSearchMode &&( logFilter == DebugLogFilter.All ||
        ...
        public void SearchButtonPressed()
        {
            ...
            searchInputField.gameObject.SetActive(isSearchMode);
            if(notSearchBtn != null)
            {
                for (int i = 0; i < notSearchBtn.Count; i++)
                {
                    notSearchBtn[i].gameObject.SetActive(!isSearchMode);
                }
            }
            if(!isSearchMode)
            {
            ...
            FilterLogs(searchInputField.text);
        }
